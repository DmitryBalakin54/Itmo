...
public class BinarySearch {
    // Pred: args[i] -> int && (int)args[args.length - 1] <= (int)args[0] && for all l, j from [1; args.length - 1] where l < j : (int)args[l] >= (int)args[j]
    ...
    public static void main(String[] args) {
        ...
        // Pred && x == (int)args[0] && for all i from a : i == 0
    ...
    // Pred1: for all l, j from [0; a.length - 1] where l < j : a[l] >= a[j] && exist k from [0; a.length - 1] : a[k] <= x
    ...
    // Pred2: for all l, j from (left; right] where l < j : a[l] >= a[j] && exist k from (left; right] : a[k] <= x && left < right
    ...
    public static int recurrentBinarySearch(int[] a, int x, int left, int right) {
        ...
        if (a[mid] <= x) {
            ...
            // check Pred2: for all l, j from (left, mid] where l < j : a[l] >= a[j] && exists k from (left; mid] : a[k] <= x (mid) && left < mid
            return recurrentBinarySearch(a, x, left, mid);
        } else {
            // Pred2 && left + 1 != right && left < mid < right && a[mid] > x
            // check Pred2: for all l, j from (mid, right] where l < j : a[l] >= a[j] && exists k from (mid; right] : a[k] <= x (a[mid] > x && Pred2 => k in [mid + 1; right]) && mid < right
