...
public class UserCredentialsEnterValidator implements Validator {
    ...
    public void validate(Object target, Errors errors) {
        if (!errors.hasErrors()) {
            ...
            User user = userService.findByLogin(enterForm.getLogin());
            if (user == null) {
                errors.rejectValue("login", "error.login.notfound", "incorrect login");
                return;
            }

            if (user.getDisabled()) {
                errors.rejectValue("login", "error.disabled", "user is disabled");
                return;
            }
